...
for (ap in daystodo) {
  ...
  gather[!cm21_bad_data_flag %in% c("empty", "pass"), Date]

  fig <- add_trace(fig,
                   x = gather[!cm21_bad_data_flag %in% c("empty", "pass"), Date],
                   y = gather[!cm21_bad_data_flag %in% c("empty", "pass"), GLB_otf],
  ...
  fig <- add_trace(fig,
                   x = gather[!chp1_bad_data_flag %in% c("empty", "pass"), Date],
                   y = gather[!chp1_bad_data_flag %in% c("empty", "pass"), DIR_otf],
                   name   = "Excluded CM21 in DB",
                   text   = paste(gather[!chp1_bad_data_flag %in% c("empty", "pass"), chp1_bad_data_flag]),
                   hoverinfo = 'text',
