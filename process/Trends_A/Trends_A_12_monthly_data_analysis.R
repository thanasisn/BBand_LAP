...
Sys.setenv(TZ = "UTC")
tic <- Sys.time()
Script.Name  <- "~/BBand_LAP/process/Trends_A/Trends_A_12_monthly_data_analysis.R"
...
for (DBn in dbs) {
  ...
  for (avar in vars) {
    ...
    ## common range
    ylim <- range(DATA[[avar]], na.rm = TRUE)
    ...
    for (amo in 1:12) {
      ...
      p <- DATA |>
        ...
        labs(x = element_blank(),
             y = bquote(.(var_name(avar)) ~ ~ group("[","%","]")),
             subtitle = paste(var_name(DBn), month.name[amo], var_name(avar))) +
        theme_bw()
      show(p)
