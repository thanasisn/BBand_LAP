...
## all runs
model_cs     <- "~/LibRadTranG/Clear_sky_model_AERONET_monthly/Model_CS_trend_fix_2.Rds"
...
## Get adjusted Brewer DATA ----------------------------------------------------

BR1 <- fread("~/MANUSCRIPTS/02_enhancement/data/AOD_340.csv")

BR1[, V1 := NULL]
BR1[, Cimel_AOD340 := NULL]
BR1 <- BR1[!is.na(Brewer_AOD340)]

## create libratran params
BR1$a         <- 1.6
BR1$b         <- BR1$Brewer_AOD340_adj * (340/1000)^BR1$a
BR1$pw_avg_mm <- 15.5

BR1[, Month := month(Date)]
BR1[, Year  := year(Date)]
...
#
# BR2 <- BR2[, .(alpha_dirspec = mean(alpha_dirspec),
#                betta_dirspec = mean(betta_dirspec),
#                a             = mean(alpha),
#                b             = mean(betta)),
#            by = year ]
