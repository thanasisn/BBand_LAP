...
Sys.setenv(TZ = "UTC")
tic <- Sys.time()
Script.Name    <- "~/BBand_LAP/process/CSid_RenoHansen/Clear_sky_id_Reno-Hansen_apply_v14.2_legacy.R"
Script.Version <- "14.2"
...
plotsbase <- paste0("~/BBand_LAP/REPORTS/DAILY/CSid_RenoHansen/",
...
for (yyyy in years) {
  ...
  strong <- LAP               |>
    ...
    collect()                 |>
    data.table()
  setorder(strong, Date)

  subdayslist  <- strong[, unique(as.Date(Date))]
  total_points <- nrow(strong)
  ...
  if (TEST == TRUE) {
    subdayslist <- sample(subdayslist, SAMPLE_DAYS)
  }
  ...
  for (aa in subdayslist) {
    ...
    if (FDP_active) {
      ...
      if (nrow(subday) > 0 & tot_p <= FDPlim ) {
        subday$CSRHv14_2_flag <- Flag_key
        subday[[paste0("CSRHv14_2_flag_", Flag_key)]] <- TRUE
    ...
    if (MeanVIP_active & any(have_glb)) {
      Flag_key <- 1L
      ## create running mean of modelled values
      ...
      if (length(indx_todo) > 0) {
        for (i in indx_todo) {
          walk(i, nt_hw , tot_p )
          subday$CSRHv14_2_flag[w_sta:w_end][ ! subday$CSRHv14_2_flag[w_sta:w_end] == 0 ] <- 0
        }
      }
      ## set MeanVIP flag
      subday[[paste0("CSRHv14_2_flag_", Flag_key)]][ subday$CSRHv14_2_flag == 99 ] <- TRUE
      subday$CSRHv14_2_flag[subday$CSRHv14_2_flag == 99]                           <- Flag_key
    }
    ...
    if (MaxVIP_active & any(have_glb)) {
      ...
      if (length(indx_todo) > 0) {
        ...
        subday$CSRHv14_2_flag[subday$CSRHv14_2_flag == 0] <- 99

        for (i in indx_todo ) {
          walk(i, nt_hw , tot_p )
          subday$CSRHv14_2_flag[w_sta:w_end][ (!subday$CSRHv14_2_flag[w_sta:w_end] == 0 ) &
                                                ( subday$CSRHv14_2_flag[w_sta:w_end] == 99) ] <- 0
    ...
    if (VIL_active) {
      ...
      if (length(indx_todo) > 0) {
        ...
        for (i in 1:tot_p) {
          ...
          ## set VIL flag
          subday$CSRHv14_2_flag[w_sta:w_end][(!subday$CSRHv14_2_flag[w_sta:w_end] == 0)  &
                                               ( subday$CSRHv14_2_flag[w_sta:w_end] == 99) &
                                               pass                                ] <- 0
    ...
    if (LDI_active) {
      ...
      subday[CSRHv14_2_flag == 0 &
               HOR_strict < MS$LDIlim,
             CSRHv14_2_flag := Flag_key]

      subday[CSRHv14_2_flag == 99 &
               HOR_strict < MS$LDIlim,
             CSRHv14_2_flag := Flag_key]

      subday[HOR_strict < MS$LDIlim,
             paste0("CSRHv14_2_flag_", Flag_key) := TRUE]
    ...
    if (LGI_active) {
      Flag_key  <- 7L
      subday[(CSRHv14_2_flag == 0 ) & (GLB_strict < MS$VGIlim), CSRHv14_2_flag := Flag_key]
      subday[(CSRHv14_2_flag == 99) & (GLB_strict < MS$VGIlim), CSRHv14_2_flag := Flag_key]

      subday[GLB_strict < MS$VGIlim, paste0("CSRHv14_2_flag_", Flag_key) := TRUE]
      subday[GLB_strict < MS$VGIlim, paste0("CSRHv14_2_flag_", Flag_key) := TRUE]
    }
    ...
    if (DST_active & any(have_dir)) {
      Flag_key  <- 11L
      subday[(CSRHv14_2_flag == 0 ) & (HOR_strict < CS_ref_HOR * ( 1 - (MS$DIR_s_T_fact / 100))),
             CSRHv14_2_flag := Flag_key]
      subday[(CSRHv14_2_flag == 99) & (HOR_strict < CS_ref_HOR * ( 1 - (MS$DIR_s_T_fact / 100))),
             CSRHv14_2_flag := Flag_key]

      subday[HOR_strict < CS_ref_HOR * ( 1 - (MS$DIR_s_T_fact / 100)), paste0("CSRHv14_2_flag_", Flag_key) := TRUE ]
    }
    ...
    if (FCS_active) {
      ...
      wecare <- grep("CSRHv14_2_flag_", names(subday), value = T)
      acount <- nrow(subday[ rowSums( subday[, ..wecare], na.rm = T) == 0, ])
      if ( acount < FCSlim ) {
        subday[ subday$CSRHv14_2_flag == 0 , CSRHv14_2_flag := Flag_key]
        subday[ subday$CSRHv14_2_flag == 99, CSRHv14_2_flag := Flag_key]
        subday[, paste0("CSRHv14_2_flag_", Flag_key) := TRUE]
      }
    }
    ...
    subday$CSRHv14_2_flag[subday$CSRHv14_2_flag == 99] <- 0

    ## Evaluation of CS detection
    clear_sky <- subday$CSRHv14_2_flag == 0
    ...
    RMSE_r <- rmse_vec(CS_ref_safe[clear_sky], subday$GLB_strict[clear_sky], na_rm = TRUE)

    MBE  <- mean(CS_ref_safe[clear_sky] - subday$GLB_strict[clear_sky], na.rm = TRUE) /
      mean(subday$GLB_strict[clear_sky], na.rm = TRUE)

    cost <- sum( ( subday$GLB_strict[clear_sky] - CS_ref_safe[clear_sky] )**2 , na.rm = TRUE) /
      sum(clear_sky, na.rm = TRUE)


    ## ID statistics
    MeanVIPcnt <- sum(subday$CSRHv14_2_flag == 1, na.rm = TRUE)
    MaxVIPcnt  <- sum(subday$CSRHv14_2_flag == 2, na.rm = TRUE)
    VILcnt     <- sum(subday$CSRHv14_2_flag == 3, na.rm = TRUE)
    VCTcnt     <- sum(subday$CSRHv14_2_flag == 4, na.rm = TRUE)
    VSMcnt     <- sum(subday$CSRHv14_2_flag == 5, na.rm = TRUE)
    ...
    ddd = subday$Date[    subday$CSRHv14_2_flag == 1 ]
    vvv = subday$GLB_strict[ subday$CSRHv14_2_flag == 1 ]
    ...
    ddd = subday$Date[    subday$CSRHv14_2_flag == 2 ]
    vvv = subday$GLB_strict[ subday$CSRHv14_2_flag == 2 ]
    points(ddd, vvv,            pch = 8, col = kcols[2], cex = .4)
    points(ddd, vvv - vvv - 15, pch = 8, col = kcols[2], cex = .2)

    ##  3. Variability in irradiance by the length (VIL)
    ddd <- subday$Date[    subday$CSRHv14_2_flag == 3 ]
    vvv <- subday$GLB_strict[ subday$CSRHv14_2_flag == 3 ]
    points(ddd, vvv,            pch = 8, col = kcols[3], cex = .4)
    points(ddd, vvv - vvv - 15, pch = 8, col = kcols[3], cex = .2)

    ##  4.  Variance of Changes in the Time series (VCT)
    ddd <- subday$Date[    subday$CSRHv14_2_flag == 4 ]
    vvv <- subday$GLB_strict[ subday$CSRHv14_2_flag == 4 ]
    points(ddd, vvv,            pch = 8, col = kcols[4], cex = .4)
    points(ddd, vvv - vvv - 15, pch = 8, col = kcols[4], cex = .2)

    ##  5.  Variability in the Shape of the irradiance Measurements (VSM)
    ddd <- subday$Date[    subday$CSRHv14_2_flag == 5 ]
    vvv <- subday$GLB_strict[ subday$CSRHv14_2_flag == 5 ]
    points(ddd, vvv,            pch = 8, col = kcols[5], cex = .4)
    points(ddd, vvv - vvv - 15, pch = 8, col = kcols[5], cex = .2)

    ##  6. Low Direct Irradiance limit (LDI)
    ddd    <- subday$Date[    subday$CSRHv14_2_flag == 6 ]
    vvv    <- subday$GLB_strict[ subday$CSRHv14_2_flag == 6 ]
    LDIcnt <- sum(subday$CSRHv14_2_flag == 6, na.rm = T)
    points(ddd, vvv,            pch = 8, col = kcols[6], cex = .4)
    points(ddd, vvv - vvv - 15, pch = 8, col = kcols[6], cex = .2)


    ##  7. Low Global Irradiance limit (LGI)
    ddd    <- subday$Date[    subday$CSRHv14_2_flag == 7 ]
    vvv    <- subday$GLB_strict[ subday$CSRHv14_2_flag == 7 ]
    VGLcnt <- sum(subday$CSRHv14_2_flag == 7, na.rm = T)
    points(ddd, vvv,            pch = 8, col = kcols[7], cex = .4)
    points(ddd, vvv - vvv - 15, pch = 8, col = kcols[7], cex = .2)


    ##  11. Too low direct radiation (DsT)
    ddd    <- subday$Date[    subday$CSRHv14_2_flag_11 ]
    vvv    <- subday$HOR_strict[ subday$CSRHv14_2_flag_11 ]
    DsTcnt <- sum(subday$CSRHv14_2_flag_11, na.rm = T)
    ...
    if (MeanVIP_active && any(have_glb)) {
      ...
      ylim <- range(GLB_rm - CS_ref_rm_base,
                    ...
                    CS_ref_rm_VIP_upp - CS_ref_rm_base, na.rm = TRUE)
    ...
    strong$CSRHv14_2_flag[sell] <- subday$CSRHv14_2_flag  ## old legacy flag?
  ...
  # export[CSRHv14_2_flag == 99, CSRHv14_2_flag := NA]

  gather <- unique(gather)

  ## rename and prepare for duckdb
  gather <- gather                            |>
    rename(CSRHv14_2_CS_ref     = CS_ref)     |>
    rename(CSRHv14_2_VIL_low    = VIL_low)    |>
    rename(CSRHv14_2_VIL_upp    = VIL_upp)    |>
    rename(CSRHv14_2_VIL_GLB    = VIL_GLB)    |>
    rename(CSRHv14_2_CS_ref_HOR = CS_ref_HOR) |>
    select(-DIR_strict,
           -GLB_strict,
           -HOR_strict,
           -SZA,
...
pander(table(strong$CSRHv14_2_flag))
#'
...
barplot(table(strong$CSRHv14_2_flag),
        main = "Flag Count")

barplot(prop.table(table(strong$CSRHv14_2_flag)),
        main = "Flag Frequency" )
#'


## Evaluation of CS detection
clear_sky <- strong$CSRHv14_2_flag == 0

RMSE <- sqrt(
  mean(
    (strong$CSRHv14_2_CS_ref[clear_sky] - strong$GLB_strict[clear_sky] )**2 , na.rm = T)) /
  mean(strong$GLB_strict[clear_sky],
       na.rm = T)
...
for (yyyy in min(year(complete$Day)):max(year(complete$Day))) {
  ...
  counts     <- table(complete$CSRHv14_2_flag[yyyyselect], complete$Day[yyyyselect] )
...
## store params
write_RDS(MS, paste0("~/BBand_LAP/SIDE_DATA/CSid_RenoHansen/", basename(sub("\\.R$", "", Script.Name))))
