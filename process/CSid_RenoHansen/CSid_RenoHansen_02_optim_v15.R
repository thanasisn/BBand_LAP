...
#'  diffuse model. The output of this procedure should be used to set-up
...
switch(
  SCHEME,
  "All" = {
    model_results_file <- "~/BBand_LAP/SIDE_DATA/model_opt/Combinations_results_"
  },
  "Monthly Clima" = {
    model_results_file <- "~/BBand_LAP/SIDE_DATA/model_opt/Combinations_results_monthly_clim_"
  },
  "Yearly" = {
    model_results_file <- "~/BBand_LAP/SIDE_DATA/model_opt/Combinations_results_yearly_"
  },
  stop("No scheme selected")
)
...
MS <- data.frame(
  ...
  completeness      = 0.3,         ## Daily completeness of data
...
flags_GHI <- DT |> select(starts_with("QCV10"), -contains("_dir_")) |> colnames()
flags_DNI <- DT |> select(starts_with("QCV10"), -contains("_glb_")) |> colnames()
cat(c("Quality flags for GHI:", flags_GHI), "\n\n")
cat(c("Quality flags for DNI:", flags_DNI), "\n\n")
...
DIR <- DT |>
  ...
  select(-contains("cm21"), -contains("_glb_")) |> # Ignore DNI data
  filter(                                        # Keep only very good GHI data
    if_all(
      starts_with("QCV10"), -contains("_glb_")   # Use all flags without DNI
      ~ . %in% c("pass", "empty")
    )
  ) |>
  select(
...
##  Reduce data that will be excluded anyway for the optimization
cat("Ignore", TEST_DT[GLB_strict < MS$VGIlim, .N], "GLB_strict as too low\n\n")
TEST_DT[GLB_strict < MS$VGIlim, GLB_strict := NA]
cat("Ignore", TEST_DT[HOR_strict < MS$LDIlim, .N], "HOR_strict as too low\n\n")
TEST_DT[HOR_strict < MS$LDIlim, HOR_strict := NA]

##  Remove days with few data
drop_glo <- TEST_DT[, .(drop = sum(!is.na(GLB_strict))/.N < MS$completeness), by = as.Date(Date)]
TEST_DT[as.Date(Date) %in% drop_glo[drop == TRUE, as.Date], GLB_strict := NA ]

drop_dir <- TEST_DT[, .(drop = sum(!is.na(HOR_strict))/.N < MS$completeness), by = as.Date(Date)]
TEST_DT[as.Date(Date) %in% drop_dir[drop == TRUE, as.Date], HOR_strict := NA ]

drop_days <- TEST_DT[, .(drop = all(is.na(GLB_strict) & is.na(HOR_strict))), by = as.Date(Date)]
TEST_DT <- TEST_DT[!as.Date(Date) %in% drop_days[drop == TRUE, as.Date], ]


test <- TEST_DT[, sum(!is.na(GLB_strict))/.N, by = as.Date(Date)]
plot(test)

test <- TEST_DT[, sum(!is.na(HOR_strict))/.N, by = as.Date(Date)]
plot(test)
...
save_file <- "~/BBand_LAP/SIDE_DATA/CSid_RenoHansen/Naive_alpha_YEARLY.Rds"
...
save_file <- "~/BBand_LAP/SIDE_DATA/CSid_RenoHansen/Naive_alpha_CLIM_MONTH.Rds"
