...
#' ---
#' title:         "Inspect GHI CHP-1 data **L1** "
...
Sys.setenv(TZ = "UTC")
tic <- Sys.time()
Script.Name <- "~/BBand_LAP/process/Legacy_CM21_R60_export.R"
...
for (YYYY in datayears) {
    ## legacy filename
    legacyout <- paste0("~/DATA/Broad_Band/CM21_H_global/Legacy_LAP_CM21_H_L1_", YYYY, ".Rds")
...
if (COMPARE) {
    listlegacy <- list.files(path   = "~/DATA/Broad_Band/CM21_H_global/",
                             pattern = "Legacy_LAP_CM21_H_L1_[0-9]{4}\\.Rds",
    ...
    for (alf in listlegacy) {
        ...
        ## load old files
        baseDT <- data.table(readRDS(paste0("~/DATA/Broad_Band/CM21_H_global/LAP_CM21_H_L1_", yyyy, ".Rds")))
        baseDT$Elevat   <- NULL
        baseDT$QFlag_2  <- NULL
        baseDT$preNoon  <- NULL
