...
Sys.setenv(TZ = "UTC")
tic <- Sys.time()
Script.Name <- "~/BBand_LAP/parameters/pressure/combine_weather_pressure_v1.R"
...
## drop na columns
ITHE <- ITHE[, which(unlist(lapply(ITHE, function(x)!all(is.na(x))))), with = F]
DAVI <- DAVI[colSums(!is.na(DAVI)) > 0]
...
ITHE$PressuremB[ITHE$PressuremB < pressure_limit_low] <- NA
DAVI$Bar[       DAVI$Bar        < pressure_limit_low] <- NA
DITH$barometer[ DITH$barometer  < pressure_limit_low] <- NA
DILA$pressure[  DILA$pressure   < pressure_limit_low] <- NA
...
## drop bad points
DITH$barometer[(DITH$Date %in% excludedataithess)] <- NA
...
## drop bad points
ITHE$PressuremB[(ITHE$Date %in% excludedataithess)] <- NA
